...
WORDLIST_FILENAME = "C:\python_practice\python_dpr2_km14_prokopiuk\words.txt"
...
def get_word_score(word, n):
    ...
    """
    word = word.lower()
    points_for_letters = [SCRABBLE_LETTER_VALUES[letter] if letter != '*' else 0 for letter in word]
    biggest = max(7 * len(word) - 3 * (n - len(word)), 1)
    return biggest * sum(points_for_letters)
...
def update_hand(hand, word):
    ...
    for letter in hand.keys():
        if hand[letter] > 0:
            new_hand.update({letter : hand[letter]})
    return new_hand
...
def is_valid_word(word, hand, word_list):
    ...
    if wildcard == -1:
        ...
        for letter in VOWELS:
            if word[:wildcard] + letter + word[wildcard + 1:] in word_list:
                temp = True
        if temp == False:
            return False
    for letter in word:
...
def substitute_hand(hand, letter):
    ...
    hand_list = list(hand.items())
    random_letter = random.choice(list(set(VOWELS + CONSONANTS) - set(hand_letters)))
    for (i, (l, n)) in enumerate(hand_list):
        if l == letter:
            hand_list[i] = (random_letter, n)
            break
    return dict(hand_list)
